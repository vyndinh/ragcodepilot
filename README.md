...
- Re-indexing: unchanged hashes and index version allow a no-op. A change, addition, deletion, or version refresh currently re-chunks and re-embeds all remaining files in that repository/language scope; stale chunks are cleaned up. A same-hash representation refresh deletes old file points before writing replacement IDs, so old IDs do not survive a version migration. Index runs publish durable state, hold one writer lock per collection, and reuse validated dense vectors from the user cache when enriched inputs are unchanged. Retry/cleanup recovery remains the next M3 increment. `--watch` uses the same pipeline.
- Existing `index`/`search` failures include actionable remedies for missing services/models, active writers, source changes, and collection setup. The walker applies nested `.gitignore` rules and negation after configured exclusions; newly ignored files are removed from the index on re-index.
...
Hybrid search is implemented: BM25 sparse vectors (`k1=0.5`, `b=0.75`) + dense vectors + Reciprocal Rank Fusion (`--mode dense|sparse|hybrid`, default `hybrid`), with additive identifier tokens and Snowball stemming on the BM25 path. Historical baseline (`baseline_v6`, 182 chunks, 23 golden queries): `hit@5 = 0.895`, `hit@1 = 0.579`, `MRR@5 = 0.673`, `recall@5 = 0.789`, `recall@10 = 0.921`. (Indexing excludes `*_test.go` by default — see [`config.yaml`](config.yaml) `skip_file_patterns`; this recovered `hit@5` from 0.789 to 0.895 by keeping test chunks out of the top-K.) Additional retrieval components are outside the current scope. The [roadmap evidence snapshot](docs/plan/mvp_roadmap.md#evidence-snapshot-and-branch-boundary) links the fresh pinned v9 self baseline (hit@5 34/35; negatives 2/4), explains calibrated negative-query scoring, and links the completed [external chi evaluation](docs/eval/external/chi-v5.2.3/README.md) and its [post-fix recheck](docs/eval/runs/2026-09-23-idfix/README.md). MCP, an agent-first pivot, and multi-repo workspace commands are removed from active scope; existing search filters remain available.
